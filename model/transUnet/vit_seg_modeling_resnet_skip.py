...
def norm1(planes, mode='bn', groups=4):
    if mode == 'bn':
        return nn.BatchNorm2d(planes, momentum=0.95, eps=1e-03)
    elif mode == 'gn':
        return nn.GroupNorm(groups, planes)
...
class DANetHead1(nn.Module):
    def __init__(self, in_channels, out_channels):
        super(DANetHead1, self).__init__()
        inter_channels = in_channels // 4
        self.conv5a = nn.Sequential(nn.Conv2d(in_channels, inter_channels, 3, padding=1, bias=False),
                                    norm1(inter_channels),
                                    nn.ReLU())
...
class PreActBottleneck(nn.Module):
    ...
    def forward(self, x):
        ...
        #         print("number2",y.shape) #
        y = self.relu(self.gn2(self.conv2(y)))
      
        #         print("number3",y.shape) #
        y = self.gn3(self.conv3(y))
        
        #         print("number4",y.shape) #

        #         y = self.relu(self.DA(y))

        y = self.relu(residual + y)

        #         print("number5",y.shape)
        return y
...
class ResNetV2(nn.Module):
    ...
    def __init__(self, block_units, width_factor):
        ...
        # 这里是图像进入的地方，主要作用是做了一次卷积，组归一化，激活以及池化
        ...
        # 这个地方是卷积神经网络的主要结构，主要由三个block组成，分别进行了卷积，激活，池化以及归一化等操作，由这两部分可以考虑将DA块替代120行部分，或者将DA块加入到三个block的后面，以完成对编码器部分的改动，编码器最后一个考虑地方可以将DA块放到Reshape部分的前后以形成底部DA块的结构。解码器部分DA块讨论在modeling.py中的Decoder部分进行讨论
